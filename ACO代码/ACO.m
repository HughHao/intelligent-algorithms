    ...
    for k=1:9
        if M(k)==1
            ...
            if y==1%显然，如果该工序为第一个工序，则M1机床时间t1直接为当前时间加上该工序的时间
                t1=t1+T(k);
                t_bz(x,y)=t1;%第32行,在程序初始化过程中已经预定义t_bz=zeros(3,3);
...
%以下进行该代结果的输出
       ...
       plot(generation,mean(T_qunti),'ro') 
       plot(generation,min_time,'ko')
       hold on
