...
clear
clc
m=15;Alpha=1;Beta=1;gama=1;Rho=0.1;NC_max=100;Q=15;W=9;qq=0.05; % #ok<NASGU>
C=[0 0
    0 -1
    0 3
    -2 -2
...
if NC==1
    ...
    if G_best_length(NC-1)>min(L)
        G_best_length(NC)=min(L);
        pos=find(L==G_best_length(NC)); 
        G_best_route(NC,1:length(Tabu(pos(1),:)))=Tabu(pos(1),:);
    else
        G_best_length(NC)=G_best_length(NC-1);
        G_best_route(NC,1:length(Tabu(pos(1),:)))=G_best_route(NC-1,1:length(Tabu(pos(1),:)));
    end
end
...
xlabel('X_coordinate of the work_station')
ylabel('Y_coordinate of the work_station')
title(['Route Plan by ACA(shortest distance:' num2str(best_length) ')'])
 figure(2)
plot(G_best_length,'b') 
hold on 
plot(length_ave,'r') 
legend('shortest distance','in average distance')
xlabel('iteraton')
ylabel('diatance')
title('comparian between shortest and average')
